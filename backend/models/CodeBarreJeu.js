...
module.exports = (sequelize) => {
  const CodeBarreJeu = sequelize.define('CodeBarreJeu', {
    id: {
  ...
  CodeBarreJeu.isAvailable = async function(codeBarre) {
  ...
  CodeBarreJeu.exists = async function(codeBarre) {
    const code = await this.findOne({ where: { code_barre: codeBarre } });
    return !!code;
  };

  // Marquer un code comme utilise et l'associer a un jeu
  CodeBarreJeu.markAsUsed = async function(codeBarre, jeuId) {
    const code = await this.findOne({
      where: { code_barre: codeBarre, statut: 'reserve' }
    });
    if (!code) {
  ...
  CodeBarreJeu.cancel = async function(codeBarreOrId, griller = false) {
  ...
  CodeBarreJeu.getNextAvailable = async function() {
  ...
  CodeBarreJeu.restore = async function(codeBarreOrId) {
    const whereClause = typeof codeBarreOrId === 'number'
      ? { id: codeBarreOrId }
      : { code_barre: codeBarreOrId };
