...
module.exports = (sequelize) => {
  const CodeBarreLivre = sequelize.define('CodeBarreLivre', {
    id: {
  ...
  CodeBarreLivre.isAvailable = async function(codeBarre) {
  ...
  CodeBarreLivre.exists = async function(codeBarre) {
    const code = await this.findOne({ where: { code_barre: codeBarre } });
    return !!code;
  };

  // Marquer un code comme utilise et l'associer a un livre
  CodeBarreLivre.markAsUsed = async function(codeBarre, livreId) {
    const code = await this.findOne({
      where: { code_barre: codeBarre, statut: 'reserve' }
    });
    if (!code) {
  ...
  CodeBarreLivre.cancel = async function(codeBarreOrId, griller = false) {
  ...
  CodeBarreLivre.getNextAvailable = async function() {
  ...
  CodeBarreLivre.restore = async function(codeBarreOrId) {
    const whereClause = typeof codeBarreOrId === 'number'
      ? { id: codeBarreOrId }
      : { code_barre: codeBarreOrId };
