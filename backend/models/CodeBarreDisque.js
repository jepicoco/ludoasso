...
module.exports = (sequelize) => {
  const CodeBarreDisque = sequelize.define('CodeBarreDisque', {
    id: {
    ...
    tableName: 'codes_barres_disques',
    timestamps: true,
  ...
  CodeBarreDisque.isAvailable = async function(codeBarre) {
  ...
  CodeBarreDisque.exists = async function(codeBarre) {
    const code = await this.findOne({ where: { code_barre: codeBarre } });
    return !!code;
  };

  // Marquer un code comme utilise et l'associer a un disque
  CodeBarreDisque.markAsUsed = async function(codeBarre, disqueId) {
    const code = await this.findOne({
      where: { code_barre: codeBarre, statut: 'reserve' }
    });
    if (!code) {
  ...
  CodeBarreDisque.cancel = async function(codeBarreOrId, griller = false) {
  ...
  CodeBarreDisque.getNextAvailable = async function() {
  ...
  CodeBarreDisque.restore = async function(codeBarreOrId) {
    const whereClause = typeof codeBarreOrId === 'number'
      ? { id: codeBarreOrId }
      : { code_barre: codeBarreOrId };
